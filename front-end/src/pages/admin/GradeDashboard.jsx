...
import { ArrowLeft, RefreshCw, Download, Filter, ChevronDown, Award, Users, BookOpen, GraduationCap, Lock, Unlock, FileText, TrendingUp } from "lucide-react";
...
const GradeDashboard = () => {
  ...
  const handleConfirmRelease = async () => {
    if (selectedSection) {
      try {
        await adminReleaseGrades(selectedSection.key);
  ...
  const calculateStats = () => {
    ...
    return { totalStudents, totalPassing, totalSections, passRate: totalStudents ? ((totalPassing / totalStudents) * 100).toFixed(1) : 0 };
  };
  ...
  const filteredSections = Object.entries(sections).filter(([key, sec]) => {
    if (sec.meta.year != year) return false;
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  ...
                  <tr className="bg-gray-50 dark:bg-gray-800">
                    {courseNames.map((course) => (
                      <>
                        <th key={course + "1"} className="p-2 border dark:border-gray-600 text-center text-xs">Sem 1</th>
                        <th key={course + "2"} className="p-2 border dark:border-gray-600 text-center text-xs">Sem 2</th>
                        <th key={course + "3"} className="p-2 border dark:border-gray-600 text-center text-xs">Avg</th>
                      </>
                    ))}
                  </tr>
                </thead>
